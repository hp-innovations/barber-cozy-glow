...
# firewalld, and there is no SELinux step on Ubuntu.
#
# USAGE EXAMPLES
# --------------
...
APP_DIR="${APP_DIR:-/var/www${DEMO_ROOT}/${APP_NAME}}"  # disk path, e.g. /var/www/demo/barbershop
...
# ---------------------------------------------------------------------------
log "Step 1/7 — Installing system packages (Nginx, Node.js 20, git)"
apt-get update -y >/dev/null
apt-get install -y nginx git curl ca-certificates gnupg >/dev/null
# Node.js 20 LTS from NodeSource (Ubuntu's default node can be too old).
...
# ---------------------------------------------------------------------------
log "Step 3/7 — Opening firewall (ufw) for web + SSH traffic"
if command -v ufw >/dev/null 2>&1; then
  ufw allow OpenSSH >/dev/null 2>&1 || true       # keep SSH open
...
echo
echo "Next: set up auto-deploy cron (every 2 min):"
echo "  chmod +x ${APP_DIR}/deploy.sh"
echo "  (crontab -l 2>/dev/null; echo '*/2 * * * * ${APP_DIR}/deploy.sh >> ${APP_DIR}/deploy.log 2>&1') | crontab -"
